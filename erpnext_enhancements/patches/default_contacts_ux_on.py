...
def execute():
	"""Default the new "Contact & Address Quick Entry" toggle ON for existing installs.
	...
	already_set = frappe.db.sql(
		...
		("ERPNext Enhancements Settings", "contacts_ux_enabled"),
	)
	if already_set:
		return

	frappe.db.set_single_value("ERPNext Enhancements Settings", "contacts_ux_enabled", 1)
