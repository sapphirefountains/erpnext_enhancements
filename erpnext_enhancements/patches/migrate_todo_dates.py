...
def execute():
	...
	todos = frappe.db.get_all(
		"ToDo", filters={"date": ["is", "set"]}, fields=["name", "date", "custom_calendar_datetime_start"]
	)
	...
	for todo in todos:
		if not todo.custom_calendar_datetime_start and todo.date:
			...
			frappe.db.set_value(
				"ToDo",
				todo.name,
				{"custom_calendar_datetime_start": start_dt, "custom_calendar_datetime_end": end_dt},
	...
	if frappe.db.exists("DocType", "Notification"):
		notifications = frappe.db.get_all(
			"Notification", filters={"document_type": "ToDo", "date_changed": "date"}, fields=["name"]
		)

		for notification in notifications:
			frappe.db.set_value(
				"Notification", notification.name, "date_changed", "custom_calendar_datetime_start"
			)
