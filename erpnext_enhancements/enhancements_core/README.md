...
|---|---|---|
| `setup_print_formats.py` | The supplier-facing **Purchase Order** print format, upserted on every `after_migrate`. Lives here because procurement has no module of its own and a Print Format needs a real Module Def. Its identifier block reads `PO-2026-00262-PRJ-00706` (ER-2026-256847) — character-for-character the PDF's filename stem, because both call the `purchase_order_document_id` jinja method; the project's readable name prints under it. Since v1.495.0 on the app-root `print_style.py` chrome (neutral stripe), and the site's Letter Head is no longer rendered. Since v1.535.0 it prints what a supplier acts on: the supplier block from `print_lookup.ps_party` (address, Attn, phone, email found past the order), DELIVER TO from `shipping_address_display` and never the `shipping_address` Link (which printed the Address record's name on 221 of 230 orders), ORDER STATUS as "Issued" or a red Draft / On hold / Cancelled warning instead of ERPNext's own words ("Closed" reads as cancelled), "Net 30" once, and quantities as "12", not "12.0". The module also holds the **superseded-format cleanup**, `disable_superseded_print_formats` (after the chrome pass, every migrate): the stock Purchase Order, procurement and — since v1.535.0 — Sales Invoice formats (`SUPERSEDED_SALES_FORMATS`: the stock, image, return and auditing formats, the three that raise TemplateNotFoundError, the POS leftover and the Regional three; Quotation and Sales Order stock formats are left alone). Disabled, not deleted: frappe keeps `disabled` when a standard format re-imports, and a deleted one comes back | `ensure_enhancements_core_print_formats`, `disable_superseded_print_formats` |
| `setup_procurement_print_formats.py` | The other five procurement documents in the **Purchase Order**'s design — **Material Request**, **Request for Quotation** (addressed to `doc.vendor`, which ERPNext sets per supplier before each render), **Supplier Quotation**, **Purchase Receipt** (a goods-received note: quantities, no money, a line to sign) and **Purchase Invoice**. Same `after_migrate` upsert, above the chrome pass. Both modules total in `print_style`'s shared `TOTAL_*` / `GRAND` styles and the dash and rule styles are held equal to the order's by `tests/test_procurement_print_formats.py`; supplier blocks come from `print_lookup.ps_party` / `ps_rfq_suppliers`, and lines print through `ps_qty` / `ps_uom` / `ps_rich` (v1.535.0). All six procurement doctypes default to their Sapphire format through Property Setter fixtures (v1.519.0), and since v1.535.0 so do Quotation, Sales Order and Sales Invoice. ERPNext's stock formats for the five are disabled on every migrate by `setup_print_formats.disable_superseded_print_formats` (`SUPERSEDED_PROCUREMENT_FORMATS`) | `ensure_procurement_print_formats` |
| `setup_sales_print_formats.py` | The three customer-facing formats — **Quotation**, **Sales Order**, **Sales Invoice** (WI-020). Same `after_migrate` upsert; composed from shared fragments so the line table and totals cannot drift across three documents. Since v1.494.0 the chrome — stripe, wordmark, display-face title, footer — comes from the app-root `print_style.py` ([docs/print-design-system.md](../../docs/print-design-system.md)), and the site's Letter Head is no longer rendered: the template draws the wordmark itself. Since v1.535.0 each is its doctype's **default** print format (`<Doctype>-main-default_print_format` Property Setter fixtures), and ERPNext's Sales Invoice formats are disabled on every migrate by `setup_print_formats.disable_superseded_print_formats` (`SUPERSEDED_SALES_FORMATS`) | `ensure_sales_print_formats` |
| `company_contact.py` | Our address and phone as printed at the top of every outgoing document — two constants, read by `print_style.letterhead()`. The letter head is a logo and nothing else, so the shared letterhead draws these itself. The address prefers the document's own company address and falls back to the constant; the phone **is** the constant, because it is stored nowhere in ERPNext. The letterhead takes the address fieldname because Purchase Order (`billing_address_display`) and the sales doctypes (`company_address_display`) disagree on the name | `COMPANY_ADDRESS_HTML`, `COMPANY_PHONE` |
...
|---|---|---|
| **ERPNext Enhancements Settings** | Project reminder recipients (`project_reminder_emails`) + maintenance billing defaults (`maintenance_fee_item`, `maintenance_services_group`) + live collaborative editing (`collab_enabled` master switch + `collab_doctypes` allowlist, child table **Collab Doctype**; seeded by the `seed_collab_doctypes` patch — toggle doctypes with no deploy, clients pick it up on next page load) + the **Desk Experience** switches (`field_description_icons_enabled` — field descriptions as a hover ⓘ icon; `field_text_wrap_enabled` — long values wrap to three lines instead of truncating, in child-table rows and on forms). Both are cosmetic, default ON, and reach the client as `frappe.boot.ee_field_description_icons` / `ee_field_text_wrap`, so flipping either needs no deploy — clients pick it up on their next page load. Each shipped with a backfill patch, because a `default` on a **new** field of a Single never reaches the row that already exists | `project_enhancements.send_project_start_reminders`, `api.maintenance_workflow`, `api.collab` + `boot.boot_session`, `feature_flags` |
...
| **Triton Settings** | External AI/telephony gateway: URL, prompts/guidelines, model IDs, Password secrets (Maps key, Twilio creds, admin webhook secret) | `api.telephony`, `triton_chat.py` — `api.gemini` no longer reads it: the Gemini Enterprise Agent Platform takes an OAuth2 token from the Drive service account, not the Maps key |
