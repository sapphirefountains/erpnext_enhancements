...
|---|---|---|
| `setup_print_formats.py` | The supplier-facing **Purchase Order** print format, upserted on every `after_migrate`. Lives here because procurement has no module of its own and a Print Format needs a real Module Def. The identifier block prints the **project** under the order number (ER-2026-256847), resolved through the `purchase_order_projects` jinja method so the sheet and the PDF filename name the job the same way | `ensure_enhancements_core_print_formats` |
