...
- **`task.sync_task_to_google_calendar` no longer exists** (removed v1.346.0, a product decision): it pushed *every* Task site-wide into one hard-coded shared calendar with no per-person filtering — whoever the calendar was shared with saw everyone's tasks. It had also never delivered an event since the Server Script migration (it named a Frappe function that does not exist; the call was fixed in v1.344.4 and the feature retired before the disabled "ERPNext Tasks" Google Calendar account was ever re-enabled). `tests/test_hooks_integrity.py` asserts the hook's absence so a stale merge cannot bring the broadcast back; a personalised per-assignee replacement, if ever wanted, is a deliberate build (per-user OAuth or a calendar DWD scope, plus update/cancel lifecycle) — see CHANGELOG 1.346.0.
