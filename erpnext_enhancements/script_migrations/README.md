...
| `task.sync_task_to_google_calendar` | `Task` `after_insert` | On task creation, pushes a Google Calendar event to a hard-coded shared calendar (through the enabled **Google Calendar** account for that calendar id — no enabled account, no sync, silently); adds a success/failure comment. |
...
- **`task.sync_task_to_google_calendar`** hard-codes the shared calendar ID as a module constant (environment-specific to Sapphire Fountains). It writes through frappe's Google Calendar integration — the enabled **Google Calendar** account whose `google_calendar_id` matches the constant ("ERPNext Tasks" on prod) supplies the OAuth token — and calls the Google API directly, because the framework has no helper for pushing an *arbitrary* event: the `google_calendar.insert_event` the migrated Server Script named never existed in any Frappe version this app ran on, so every sync since the migration died with `AttributeError` (v1.344.4). While the account is disabled the sync skips silently: a deliberately switched-off integration must not leave a "contact your system administrator" comment on every Task.
