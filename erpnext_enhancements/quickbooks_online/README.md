...
| `core/party_group_remediation.py` | One-off (v1.496.0): put each QBO-linked Supplier / Customer back in the group it held before the importer's default-group sweep, read from `tabVersion`; Supplier search fields recomputed alongside. Dry-run by default, **never wired to migrate/scheduler** -- see [Gotchas](#gotchas) and `MIGRATION_NOTES.md` §7 | `restore_party_groups`, `pre_sweep_group`, `restoration_target`, `SWEEP_LANDING_GROUPS` |
...
- **A party's group / territory is ERPNext's, and the importer's default for it is a NAME.** QuickBooks has no supplier group, customer group or territory, so `_map_supplier` / `_map_customer` file a *new* party into the seeded `Uncategorized` leaf (`constants.DEFAULT_PARTY_GROUP`, created by `patches/seed_qbo_uncategorized_groups`). Until v1.496.0 the default was `frappe.db.get_value(doctype, {"is_group": 0}, "name")` -- and a dict-filtered `get_value` on Frappe 16 orders by `creation` **descending**, so "any leaf" meant "the leaf somebody created most recently"; because the update path re-applied every mapped value on each re-sync, every Supplier Group anyone added became the group of all 911 QBO-linked Suppliers on the next scheduled run (five sweeps between 2026-06-18 and 2026-09-16), and 466 Customers landed in "Government" -- invisibly, since `customer_group` is hidden on the Customer form. Now `_protect_existing_party_groups` drops the field from an update when the record already has one, and `ERPNEXT_OWNED_PARTY_FIELDS` are excluded from the `owned_fields` snapshot and from `detect_conflicts`, so re-grouping a Supplier is never a conflict. The swept records are restored by `core/party_group_remediation.py` (dry-run by default; `MIGRATION_NOTES.md` §7). Never resolve a default with an unordered `get_value` -- name it.
