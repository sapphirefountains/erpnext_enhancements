...
| `core/party_group_remediation.py` | Remediation (v1.496.0): put each QBO-linked Supplier / Customer back in the group it held before the importer's default-group sweep (from `tabVersion`) or **clear** it, plus the audit's hand-curated corrections in `core/supplier_group_corrections.json`; Supplier search fields recomputed alongside. Dry-run by default. The Supplier side runs once from `patches/restore_supplier_groups_after_qbo_sweep`; the Customer side is by hand -- see [Gotchas](#gotchas) and `MIGRATION_NOTES.md` §7 | `restore_party_groups`, `apply_curated_supplier_groups`, `load_curated_supplier_groups`, `pre_sweep_group`, `restoration_target`, `SWEEP_LANDING_GROUPS` |
