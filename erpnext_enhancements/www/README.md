...
- **`/pay-card`** (`pay_card.py` + `pay-card.html`): the review step is a history entry of its own (`pushState(state, "")`, so `?invoice=` never changes), the phone's Back returns to the card step, and Forward shows the review again until the card changes; any failed charge spends its quote and returns to the card step. A paid invoice — or one whose payment is still settling, or one with a Stripe payment already received — renders "paid" / "being processed" / "received" with a link back to `/pay` instead of a card form. The Stripe README's [card page section](../stripe_payments/README.md#the-card-page-pay-card-back-forward-and-paying-twice) has the rules and why.
