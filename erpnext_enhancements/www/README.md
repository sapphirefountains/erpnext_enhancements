...
**Back and Forward walk the trips.** The trip on screen is in the address as `?trip=<name>` (a query string on the same path, so no route rule; `trip` is clear of the keys Frappe consumes). A trip chip tap pushes one entry, from the tap itself, and Back / Forward arrive as `popstate`, which loads that entry's trip and pushes nothing: `loadTrip` never writes history, and its race guard drops a response — or an error — for a trip no longer on screen. At boot, `?trip=` is accepted only when it names one of the person's active trips; when it is missing or unknown, the entry the page opened on is *replaced* with the default trip (current, else next, else first), never pushed, so Back from it leaves the page — or the home-screen app — as before. A reload, Back from `/travel_guidelines` and a login all keep the trip: `itinerary.py` sends a signed-out visitor to `/login?redirect-to=<full path>`, encoded whole (as Stock Scan does). "Report a problem" pushes and answers its own entry (`capture/panel.js`), and the page leaves `popstate` alone while `ee_capture.isOpen()`, then shows whatever trip the entry the panel left it on names. Trip emails and calendar invites still link to bare `/itinerary`. `scripts/test_web_flow_history.js` drives the real `itinerary.js` over a fake history; `tests/test_travel_planner.py` runs it.

## Customer pages: `/pay-card` and `/contract-sign` — Back and Forward

Both are public-facing (`/pay-card` login-gated to portal customers, `/contract-sign` a bearer link) and both carry no capture code (`tests/test_feedback_capture_surface.py`).

- **`/pay-card`** (`pay_card.py` + `pay-card.html`): the review step is a history entry of its own (`pushState(state, "")`, so `?invoice=` never changes), the phone's Back returns to the card step, and Forward shows the review again until the card changes. It reaches the server over `fetch` (`/api/method/…` with the CSRF token), never `frappe.call`, whose website build calls back on a 200 only and never calls `error()` — which is how a declined card once left Pay stuck on "Please wait…". A definite failure (the server refused or Stripe declined — a 4xx naming its `exc_type`) spends its quote and returns to the card step with the server's message; a refusal for the invoice rather than the card (`PaymentBlocked`: another payment settling, the invoice paid or credited) reloads instead, so the render says why and no live card form sits beside "already being processed" — except its subclasses `LinkStillOpen` and `AttemptUnreleased` (an earlier attempt Stripe would not confirm canceled), which a render cannot show, so the form stays and says why; a session the page no longer has (a stale CSRF token, or a sign-in that expired or ended elsewhere) reloads as well, for a new token or the login page; **no answer is never a failure** — a dropped connection, a 5xx, a timeout, 3-D Secure ending in anything but a card or request error may have charged the card, so the page holds Pay and reloads for the server's verdict instead of showing a card form. A paid invoice — or one with nothing left to pay, one whose payment is still settling, one with a Stripe payment already received, one whose card payment is waiting on the payer's bank, one whose charge nobody has an answer for yet, one with a bank debit waiting on the account's verification, or one held by an off-session card attempt Stripe would not confirm canceled — renders "paid" (only when ERPNext says Paid) / "nothing left to pay" / "being processed" / "received" / "waiting for your bank's approval — you can pay again in about N minutes" / "we could not yet confirm it — please do not pay again" / "waiting for your bank account to be verified" / "could not be released just now — try again in a few minutes", with a link back to `/pay` instead of a card form; only "being processed" promises to show as paid once it clears. Renders wait at most a few seconds per Stripe lookup and read a failed one as not yet confirmed. **`/stripe-return`** (`stripe_return.py` + `stripe-return.html`) says what the ledger row says now, not what the address says: received, being processed, not yet confirmed (do not pay again for now; if it did not go through we will email you, or you will find the invoice ready to pay again — including an attempt poll_pending has found but not seen complete), or did not go through (nothing was charged). The Stripe README's [card page section](../stripe_payments/README.md#the-card-page-pay-card-back-and-forward) and [one invoice, one payment](../stripe_payments/README.md#one-invoice-one-payment) have the rules and why. Its list page, **`/pay`** (`pay.py` + `pay.html`), offers Card and Bank exactly where the endpoints behind them would accept a payment (`card_element.invoice_payment_blocks`, the same short-timeout, read-only verdict, which also reads the invoice an amended one was made from): any verdict hides both — "Payment received — please contact us", "Waiting for bank approval…" or "Processing…". Its Bank button expires any Checkout link already sent for the invoice before it opens a new one.
- **`/contract-sign`** (`contract_sign.py` + `contract-sign.html`, `public/js/contract_sign/`): **no history entry, deliberately** — the executed form must never come back, so signing and declining both swap in place. (Declining used to reload, and because a decline retires the link, the reload said "This link isn't available".) "Save a card" is a real navigation to Stripe, and Back from Stripe reloads the link into "Already signed". That notice now offers the enrolment again: `contract_sign.js` keeps the Stripe link this tab was handed in `sessionStorage` (filed under a fingerprint of the ref, never the ref; gone with the tab), and shows it only while `_autopay_resumable` says the enrolment was started and has not finished. Nothing is minted or fetched from the link alone, so a forwarded link still cannot open a card enrolment (`esign.portal._signed_session`). A copy restored from the back-forward cache re-enables "Save a card". `scripts/test_web_flow_history.js` drives it; `tests/test_contract_esign.py` runs it.

**"View invoice (PDF)"** is on `/pay` under every invoice's name, whatever the row's payment
state, and on `/pay-card` beside the invoice header, on the card form and on every settled state.
It is not on "not available", which names no invoice, and not on `/pay` while Stripe Settings is
switched off: that page then says online payments are unavailable and lists no invoices at all.
The endpoint itself does not look at the switch. Both pages link to
`stripe_payments.core.api.portal_invoice_pdf?invoice=<name>` with `target="_blank"
rel="noopener"`: a new tab, so the phone's PDF viewer opens it and the list, or a card already
typed in, stays where it was. It is a plain link, not a `frappe.call`, and the endpoint checks
ownership again. The Stripe README's
[invoice PDF section](../stripe_payments/README.md#the-invoice-pdf-on-pay-and-pay-card) has the
rules.
