...
- **`website` is a URL field because *we* made it one, and that froze 384 records.** Stock ERPNext ships `website` as a plain `Data` field on Lead / Customer / Opportunity / Supplier / Company; five Property Setter fixtures (`*-website-options`) set `options = "URL"`, which switches on frappe's `validate_url`. It arrived after the data did. frappe re-validates **every** URL field on **every** save, so a stored `2xlimaging.com` rejected an edit to the phone number, the customer group, or anything a background job touched — 281 of 739 Customers, 80 of 506 Opportunities, 14 of 35 Leads, 9 of 16 Lead `custom_account_website` values (plus 24 Suppliers, still unwired). `website_cleanup.add_missing_scheme` fixes it on the way in and `patches.backfill_website_scheme` repaired the stored values once. If you are tempted to drop the Property Setters instead: that also works, and loses the validation — the fix here keeps it and stops it being a trap.
