import frappe
from frappe.utils import add_days, add_months, add_years, getdate, get_weekday
...
def create_duplicate_task(doc, new_date):
    ...
    frappe.msgprint(f"Scheduled next task for {new_date}: <a href='/app/task/{new_doc.name}'>{new_doc.name}</a>")
