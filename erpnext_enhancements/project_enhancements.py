...
def _get_user_details(user_list):
	"""
	Fetches full name and user image for a list of users in a single query.
	"""
	if not user_list:
		return {}
	...
	user_details = frappe.get_all(
		"User",
		filters={"name": ["in", list(set(user_list))]},
		fields=["name", "full_name", "user_image"],
	)

	return {user.name: user for user in user_details}


@frappe.whitelist()
def get_project_comments(project_name):
	"""
	Fetches comments for a given project, along with user details.
	"""
	if not project_name:
		return []
	...
	owners = [c.owner for c in comments]
	user_details_map = _get_user_details(owners)

	# Enhance comments with user details
	for comment in comments:
		user_info = user_details_map.get(comment.owner)
		if user_info:
			comment.full_name = user_info.get("full_name")
			comment.user_image = user_info.get("user_image")
		else:
...
def add_project_comment(project_name, comment_text):
	...
	if not project_name or not comment_text:
		frappe.throw(_("Project Name and Comment are required."))
	...
	# Create a dictionary from the new comment doc
	new_comment_data = comment.as_dict()

	# Fetch user details for the new comment's owner
	user_details_map = _get_user_details([comment.owner])
	user_info = user_details_map.get(comment.owner)

	# Enhance with user details
	if user_info:
		new_comment_data["full_name"] = user_info.get("full_name")
		new_comment_data["user_image"] = user_info.get("user_image")
	else:
		new_comment_data["full_name"] = "Unknown"
		new_comment_data["user_image"] = None
