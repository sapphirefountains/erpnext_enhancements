...
- **The Schedule tab's Export dropdown carries a "Date range…" entry** (v1.441.0): a dialog with presets (whole schedule, remaining from today, next 30/90 days, this month, this quarter) plus From/To and a format, covering all five outputs. It narrows the rows *and* the calendar — see the [public README](../public/README.md) for why bars that cross an edge are clipped rather than clamped. The Projects Dashboard's portfolio Gantt owns its own toolbar markup and does **not** get the entry; the capability is there (`widget.export_range_dialog()`, or `range` on any export call) if that toolbar ever wants it.
