...
- **Page:** a standard Frappe Page named `project-dashboard` (module "Projects"). `project_dashboard.js` builds an app-page shell with header actions (**New Project** / **New Master Project** quick-create) and tabs — Priority Overview (default landing), Active Internal Projects, Completed Projects, Portfolio Gantt, Tasks View, **Dashboard** — routing via `frappe.set_route("project-dashboard", <tab>)`. Each tab is a separate component class lazy-loaded with `frappe.require` from `public/js/project_enhancements/dashboard_components/`. NOTE: those `frappe.require` URLs must include the `project_enhancements/` path segment (`/assets/erpnext_enhancements/js/project_enhancements/dashboard_components/…`) to match where the files live; the short form 404s.
- **Data source:** the whitelisted methods in `project_dashboard.py`. `get_project_data` uses bulk SQL/`get_all` for task counts and derives assignees from **ToDo** rows (Project has no `project_user` column — selecting one would raise "Unknown column"). `get_dashboard_metrics` returns the portfolio aggregates (headline cards + status/type/completion breakdowns) that back the native **Dashboard** tab. The Active Internal Projects tab shows only active projects whose `project_type` is in `INTERNAL_PROJECT_TYPES` (server-side constant, mirrored client-side).
