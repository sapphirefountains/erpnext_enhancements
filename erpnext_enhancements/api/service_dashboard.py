...
"""Whitelisted feeds for the Service Dashboard widgets.
...
their ``operations_*`` fieldnames on purpose, because renaming a field on a Single
drops the value every site has already stored.

Gating is the shared department contract in ``api/dashboard_widgets.py``:
Service roles per ``api/kpi.py``, plus a per-widget toggle defaulting OFF.
"""
...
@frappe.whitelist()
@widget_feed("Service", "chemistry_alerts")
...
@frappe.whitelist()
@widget_feed("Service", "labor_capture")
