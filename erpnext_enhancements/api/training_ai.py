...
# ----------------------------------------------- whole-course authoring (Triton)
#
# The floating trident on the training SPAs lands here. The division of labour is
# the one the AI-authoring ADR records and the rest of this module already lives
# by: the model *proposes* a Course Spec, and the deterministic materialiser in
# api.training_course_authoring *builds* it -- and never publishes. So the worst a
# bad draft can do is leave an unpublished course for the author to fix or delete;
# nothing reaches a learner without a human pressing publish. Unlike the per-block
# drafters above (suggestions an author accepts row by row), this mints a whole
# course at once, which is exactly why it stops at a review-gated draft.

# The most a brief may hold. A course brief is a paragraph, not a manuscript: a
# model handed an essay writes a worse course, not a better one, and the token
# bill is the author's to pay twice on a validation retry.
MAX_BRIEF_CHARS = 4000


@frappe.whitelist(methods=["POST"])
def draft_course_with_triton(brief):
    """Draft a whole Training Course from a natural-language brief.

    Returns ``author_course_from_spec``'s payload (``{course, course_title,
    draft_version, url, ...counts}``) plus a one-line ``summary`` and the
    ``ai_model`` that drafted it. Raises with an author-actionable message and
    creates nothing if the model's output will not validate as a Course Spec.
    """
    _require_author()
    _require_ai_enabled()
    ...
    text = (brief or "").strip()
    if not text:
        frappe.throw(_("Tell Triton what the course should teach."))
    if len(text) > MAX_BRIEF_CHARS:
        frappe.throw(
            _("That brief is longer than this box is meant to hold. Trim it to {0} characters.").format(
                MAX_BRIEF_CHARS
            )
        )

    # Lazy, and for the reason the FAC contract stub documents: course_spec guards
    # its own imports, and training_course_authoring pulls in frappe.model, so
    # neither may be a module-scope import of this drafting file.
    from erpnext_enhancements.training import course_spec as spec_mod
    from erpnext_enhancements.api.training_course_authoring import author_course_from_spec

    system = _course_author_system_instruction(spec_mod)
    spec = _draft_and_validate_spec(text, system, spec_mod)

    result = author_course_from_spec(spec)
    result["summary"] = spec_mod.spec_summary(spec)
    result["ai_model"] = _model_id()
    return result


def _course_author_system_instruction(spec_mod):
    schema = json.dumps(spec_mod.COURSE_SPEC_SCHEMA, default=str)
    return (
        "You are Triton, an instructional designer for an internal compliance-training LMS. "
        "From the author's brief, design ONE course and return it as a JSON object that conforms "
        "EXACTLY to this JSON Schema:\n\n" + schema + "\n\n"
        "Hard rules:\n"
        "- Output JSON only: no prose, no explanation, no code fences.\n"
        "- Use ONLY the block types the schema's enum allows. Never invent a Video, Image, PDF or "
        "Embed block -- a model cannot supply the media, and a human adds those in the builder.\n"
        "- Write real teaching content in the blocks, not placeholders or 'TODO'.\n"
        "- A short course is a good course: a handful of focused lessons unless the brief clearly "
        "asks for more.\n"
        "- Where you add a quiz, write plausible distractors and mark the correct option(s)."
    )


def _draft_and_validate_spec(brief, system, spec_mod):
    """Draft, validate, and -- once -- self-correct. Returns the validated spec.

    The single corrective retry hands the model its own output and the reason it
    was rejected. It is worth exactly one round trip: a model that cannot produce a
    valid spec twice is not going to on the third try, and the author is waiting.
    """
    parsed = _parse_course_spec_json(_ask_model("Author's brief:\n" + brief, system, feature="training_course_authoring"))
    try:
        return spec_mod.validate_course_spec(parsed)
    except Exception as first_error:
        fix = (
            "Your previous JSON was rejected for this reason: " + str(first_error)[:300] + "\n\n"
            "Here is what you returned:\n" + json.dumps(parsed)[:6000] + "\n\n"
            "Return corrected JSON only, conforming to the schema."
        )
        try:
            return spec_mod.validate_course_spec(
                _parse_course_spec_json(_ask_model(fix, system, feature="training_course_authoring"))
            )
        except Exception as second_error:
            frappe.throw(
                _("Triton's draft did not fit the course format ({0}). Nothing was created -- try "
                  "rewording the brief, or build the course by hand.").format(str(second_error)[:180])
            )


def _parse_course_spec_json(text):
    """The model's reply, reduced to the JSON object it should have been."""
    raw = (text or "").strip()
    if raw.startswith("```"):
        # A fence despite the instruction not to. Strip the opening ```lang and the close.
        raw = re.sub(r"^```[a-zA-Z]*\s*", "", raw)
        raw = re.sub(r"\s*```$", "", raw).strip()
    # Leading/trailing prose despite the instruction: keep the outermost object.
    ...
    try:
        ...
        frappe.throw(_("Triton did not return a course. Nothing was created -- try again."))
