	...
	render_comments_section: function (frm) {
		try {
			if (typeof window.Vue === 'undefined') {
				frappe.msgprint("Error: Vue is not defined.");
				return;
			}

			const wrapper = frm.fields_dict["custom_comments_section"].wrapper;
			if (!wrapper) {
				console.warn('Field "custom_comments_section" not found.');
				return;
			}
			$(wrapper).html('<div id="project-comments-app"></div>');
				...
				methods: {
					fetchComments() {
						...
						frappe.call({
							...
							error: () => {
								this.isLoading = false;
								frappe.msgprint("Failed to load comments.");
							}
						});
					},
					addComment() {
						...
						frappe.call({
							...
							callback: (r) => {
								if (r.message) {
									this.comments.unshift(r.message); // Add to the top
									this.newComment = '';
								}
							}
						});
					...
					getUserAvatar(user_image, full_name) {
						if (user_image) {
							return `<img src="${user_image}" class="avatar avatar-medium">`;
						}
						return `<span class="avatar avatar-medium">${frappe.get_abbr(full_name)}</span>`;
					}
				},
				template: `
                    <div class="project-comments-section">
                        ...
                        <div class="new-comment-field">
                            <textarea v-model="newComment" class="form-control" placeholder="Add a note..."></textarea>
                        ...
                        <ul v-else class="comment-list">
                            <li v-for="comment in comments" :key="comment.name" class="comment-item">
                                <div class="comment-avatar" v-html="getUserAvatar(comment.user_image, comment.full_name)"></div>
                                <div class="comment-content">
                                    <div class="comment-header">
                                        <span class="comment-user">{{ comment.full_name }}</span>
                                        <span class="comment-time text-muted">{{ formatDateTime(comment.creation) }}</span>
                                    </div>
                                    <div class="comment-body">{{ comment.content }}</div>
                                </div>
                            </li>
							<li v-if="comments.length === 0" class="text-muted text-center" style="padding: 20px;">
								No notes yet.
							</li>
                        </ul>
                    </div>
			...
			console.error(e);
			frappe.msgprint("Error rendering comments section: " + e.message);
		}
	},
