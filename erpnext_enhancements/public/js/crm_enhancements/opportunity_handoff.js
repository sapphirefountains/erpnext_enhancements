 ...
 *  - With a linked Project (`custom_created_project`), the three rows mirror that
 *    Project's actual step statuses (steps 1-3) via a whitelisted server call.
 *  - Without one, they're derived from the Opportunity: "Mark Won" completes when
 *    the status is Closed Won; the others are upcoming.
 *
...
(function () {
	...
	function render(frm) {
		...
		const steps = [
			...
			{ no: 2, title: __("Hold Hand-Off Meeting"), status: "Pending", current: false, meta: __("After the project is created") },
			{ no: 3, title: __("Create Project in PM System"), status: "Pending", current: won, meta: won ? __("Use the Create project prompt") : "" },
		];
		paint(field, steps);
	}
