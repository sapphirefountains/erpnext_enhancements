 ...
 * 3. A third surface, the Google Chat relay, got the raw markdown, because Chat's own bold
 *    is a *single* asterisk. That leg and its server-side transform went with the chat
 *    module in v1.424.0 (ADR 0011); only the browser half — this file — survived, and it is
 *    now the widget's one and only markdown path.
 *
 * So the fix could not be "call `frappe.markdown` in more places": that spreads an unsanitised
 * `innerHTML` sink to more surfaces, and it was never available on all of them anyway.
 *
