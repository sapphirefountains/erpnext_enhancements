...
| `item_naming_check` | Inventory | a proposed Item Code/Name against the naming SOP — duplicates, scored neighbours, code family, `PDT-`/`SRV-` block occupancy, every mechanical name defect, and a STOP/FIX/PASS verdict. Advisory: nothing this tool reports blocks a save by itself. Since v1.532.0 the `Item` doc_event refuses a *new* Item for two of these findings only (a code duplicating an existing one after normalisation, a name that is just the code) |
