...
| `../workspace_sidebar/time_kiosk.json` + `../patches/seed_time_kiosk_desktop_icon.py` | The **Time Kiosk** tile on the Desk home grid (v1.481.0): an External-link `Desktop Icon` straight to `/kiosk`, and the one-item sidebar without which `get_desktop_icons` would silently drop it |
