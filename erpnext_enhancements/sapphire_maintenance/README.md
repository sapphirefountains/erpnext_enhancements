...
Technicians fill visits through the **Visit Wizard** (`/desk/visit-wizard`) — a guided, touch-first desk page (steppers, segmented buttons, tap checklists; no grids) that reads and writes the same Maintenance Record, so the desk form stays the supervisor/review surface and all automation is shared.
...
│   visit_date (the day the work was ACTUALLY done — blank means "the day it was submitted"; set
│   explicitly by create_visit and what the cadence roll-forward keys on),
...
| `doctype/sapphire_maintenance_record/sapphire_maintenance_record.html` | Portal view (`/maintenance-records/<name>`); the print format is the fixture, see below | header, Service Duration, checklist, chemistry, cleaning tasks, consumed consumables, sign-off |
...
| `page/visit_wizard/visit_wizard.js` | Visit Wizard desk page (tech touch-first flow) | step renderers (safety/chemistry/steppers/inspection/cleaning/wrap-up), per-feature tabs, autosave (+ saves parked against their own visit), signature pad, one history entry per screen (`handle_route`/`push_route`) |
...
**Visit Wizard** (`/desk/visit-wizard/<record>[/<step>[/<serial>]]`, `?record=…` still accepted, or no record for the picker): Safety briefing + PPE acknowledge gate → Water Chemistry (numeric cards with range chips; out-of-range turns red with the server's verdict on autosave) → Chemicals Used (pre-listed from the template; `[−] qty [+]` steppers stepping by `qty_step`, first tap jumps to `default_qty`; ad-hoc item add) → Inspection (segmented buttons honoring custom per-question options; Fail/Replace reveal notes + photo) → Cleaning (tap checklist) → Wrap-up (notes + dictation, signature pad, workflow-aware Finish). Per Site Visit records get sticky per-feature tabs. Steps autosave with optimistic locking (`api/maintenance_visit.py`); a stale desk-side edit rejects the write instead of losing it.

**Back and Forward.** Every screen — the picker, each step, each feature tab — is its own browser history entry, so the phone's Back goes back one screen and Forward restores it; Back from the first screen leaves the page (from the kiosk, which opens the wizard in a new tab, that closes the tab). The page moves with `frappe.set_route` and reads the route back in `on_page_show`; frappe owns history, so there is no popstate listener. The wizard's own Back button steps back through history when the entry behind is the previous step, so Next/Back/Next piles nothing up. A move from Back/Forward commits the field being typed in and flushes the autosave first, like a button tap; it honours the safety gate (so do the step dots now), and an address that cannot be shown as-is (a later step before the tick) is corrected in place. So is one that never named the visit: `frappe.set_route("visit-wizard", {record})` and an in-desk `?record=` link reach the page through `frappe.route_options` while v16 pushes the bare `/desk/visit-wizard`, which Back or a reload would read as the picker — the load replaces it with `/desk/visit-wizard/<record>/<step>` (the kiosk's full-load `?record=` entry names the visit already, and is left alone). A save still failing when the technician moves to the list or another visit is **parked against its own record** and retried there — never merged into the visit now on screen — and put back on screen if that visit is opened again first. Those saves are sent `silent`, and only a failure with no answer (or a server error) is retried: a refusal — typically "out of date" after a save that committed but whose answer was lost, so every retry carries the same stale `modified` — used to pop frappe's unnamed "Visit Out of Date … Reload" dialog over whichever visit was open, up to six times, where tapping that page's Reload dropped *its* unsent edits. Now the edits stay parked and one alert names the visit. Saves of one visit go out one at a time, each carrying the `modified` the previous one returned. A drawn client signature survives leaving Wrap-up and coming back. `scripts/test_wizard_back_forward.mjs` runs the real page against a port of the v16 router (run by `tests/test_visit_wizard_markup.py`).

**Telling the technician where they stand.** Three things the wizard used to know and never said, all of which cost a return trip:
- *Save state* — autosave is no longer silent. A persistent indicator reads Unsaved changes / Saving / Saved / **NOT SAVED — retrying**, a failed save retries itself on exponential backoff, and closing the tab with unsaved edits warns first. Before this a failed patch was re-queued and the error rethrown into nothing, so a bad signal looked exactly like a saved form.
- *Required rows* — `is_mandatory` rode from the template to the `before_submit` gate without ever being rendered. Mandatory rows now carry a Required chip that flips to a tick when answered, the step header counts what is outstanding, and a per-step bar strip shows *which* steps still owe answers. `VZ_ANSWERED` in the wizard mirrors `_validate_mandatory_rows` on the server and **the two must stay in step** — drift means the wizard reports a step finished that submit then refuses.
- *Which feature* — per-feature tabs label themselves from the Serial No's `item_name` (`get_visit_bootstrap` returns a `features` map) rather than the raw docname, and each tab carries a count of required rows outstanding *for that feature*, so a two-fountain site cannot look finished because the fountain in front of you is.

The Access & Site banner omits empty and `N/A` values, and the whole banner when nothing useful remains — printing "Code: N/A" trains people to skim past the panel that also carries the hazards.

The wizard's no-param picker shows two lists plus an always-present escape hatch. **Not on the list?** → **Log a visit** (`get_loggable_sites` → `create_visit`) offers *every* Active contract with covered features, with **no due-date filter at all**, and a date field defaulting to today that accepts any day back to `MAX_BACKFILL_DAYS` (60). That covers both "I'm at a fountain and nothing was scheduled" and "I forgot to fill Tuesday's form in". Unlike **Do Visit Today**, a record made this way carries **no `visit_label`** — it *is* the visit, so it must advance the cadence, and it advances it from `visit_date` rather than from the day the form was typed. Picking a site that already has an open draft opens that draft instead of making a second. The two lists are **Today's Visits** (open drafts, `get_my_visits_today`) and **Upcoming — do one early** (`get_upcoming_visits`): Active-contract features due in the next 8–30 days with no draft yet (Per Site Visit contracts collapse to one earliest-due site entry). Tapping **Do Visit Today** calls `create_visit_today`, which spins up a record dated today carrying the `EXTRA_VISIT_LABEL` ("Extra Visit") `visit_label` — a labelled visit, so `update_next_visit_dates` skips it: the pull-forward is an **extra one-off** and the feature's originally scheduled visit still fires on its own date.
...
- **Print format:** "Maintenance Record Print" — a Jinja fixture (`fixtures/print_format.json`) on the print design system's chrome via the `ps_*` Jinja globals ([docs/print-design-system.md](../../docs/print-design-system.md)); it is the PDF `api/maintenance_workflow.py` emails to the customer on finalize, so it prints no labor hours. The `.html` above is the **portal** view only. Guarded by `tests/test_print_style.py`, which renders the fixture against sample records.
...
- **Never write the wizard's address by hand.** It once wrote `/app/visit-wizard?record=…` with `history.replaceState`; the v16 router strips only `desk/`, so Back or Forward onto that entry parsed as a Page called `app` and showed "Page not found" (a reload worked, via frappe's server-side `/app` → `/desk` redirect, which is why it survived). Route with `frappe.set_route` segments. The kiosk's and the record form's `/app/visit-wizard?record=` links still work — they are full page loads, redirected by the server.
...
- **`visit_label` is a cadence switch, not a caption.** `update_next_visit_dates` returns early on *any* truthy label, so giving a record a label — for display, for grouping, for anything — silently stops it advancing the schedule. That is deliberate for seasonal visits and for `create_visit_today`'s "Extra Visit" pull-forward, and it is why `create_visit` (unscheduled / backfilled) leaves the label empty. If you ever want a labelled visit that *does* advance the cadence, the early return is what you have to change.
- **An empty `covered_features` table breaks a contract in two places at once, and both fail silently.** The scheduler builds `due_features` only from that table (so a Per Site Visit contract drafts nothing, forever), and `get_visit_payload` loops it to emit rows (so a record opened by hand instantiates an *empty form*). The contract looks perfectly healthy — Active, template set, frequency set. A feature row needs a **Serial No**, so the real prerequisite is one Serial No per water feature with `custom_project` set and `item_code` matching Settings → `water_feature_item`.
- **A `default` on `visit_date` would be actively wrong.** It is a normal doctype, so MariaDB writes a new column's default into *every existing row* — stamping a service date onto historical records that never had one, and feeding it to the cadence roll-forward. Blank must keep meaning "unknown, use the submission date". (See the CLAUDE.md gotcha; Singles behave the opposite way.)
