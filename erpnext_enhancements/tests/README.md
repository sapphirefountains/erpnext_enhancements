...
| `test_po_order_stage.py` | Purchase Order Order Stage: the backfill rule (drafts, cancelled, closed/completed, part-received, `None` per_received, and that every result is a real Select option), the Purchase Receipt advance/revert hooks — including which prior stages earn a timeline note when a partial receipt overwrites them, and that a `Received` order is never walked backwards — and the wiring: both patches building the field from one `field_definition()`, patch order, `allow_on_submit`, nothing on PO submit, the backfill never keying on emptiness, and the options patch carrying no data migration | **Bench-free**: same `setUpModule` stub pattern; the stub's `db.set_value` records `update_modified` so the no-TimestampMismatch promise is testable |
