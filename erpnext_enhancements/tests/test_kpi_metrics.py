...
class TestCombineStoreRuns(unittest.TestCase):
	...
	def test_the_charge_two_days_later_is_the_same_trip(self):
		"""The bank feed can date a card charge after the purchase: Home Depot $43.31 was
		2026-07-07 as a receipt entry and 2026-07-09 from the feed. Exact-day matching counted 2."""
		receipts = [self.receipt("2026-09-07", "sr-a", 40.0, total=43.31)]
		self.assertEqual(self.combine([self.charge("2026-09-09", 43.31)], receipts), (1, 43.31))
		self.assertEqual(self.combine([self.charge("2026-09-10", 43.31)], receipts), (1, 43.31))

	def test_a_charge_four_days_later_is_another_trip(self):
		receipts = [self.receipt("2026-09-07", "sr-a", 40.0, total=43.31)]
		self.assertEqual(self.combine([self.charge("2026-09-11", 43.31)], receipts), (2, 86.62))
	...
	def test_an_amount_match_beats_a_nearer_day(self):
		receipts = [self.receipt("2026-09-03", "sr-a", 40.0, total=43.31)]
		charges = [self.charge("2026-09-03", 7.0), self.charge("2026-09-05", 43.31)]
		# sr-a takes the $43.31 two days later; the $7 the same day is its own trip.
		self.assertEqual(self.combine(charges, receipts), (2, 50.31))
