...
def merge_projects(source_project, target_project):
            ...
            if docs_to_update:
                ...
                for doc_name in docs_to_update:
                    try:
                        ...
                        frappe.log_error(f"Failed to update {doctype} {doc_name}: {str(e)}", "Project Merge Error")
    ...
    return _("Successfully merged {0} documents from {1} to {2}.").format(updated_count, source_project, target_project)
...
def get_linked_doctypes(doctype_name):
    ...
    link_fields = frappe.get_all("DocField", filters={"fieldtype": "Link", "options": doctype_name}, fields=["parent", "fieldname"])
    
    # Also check Custom Fields
    custom_link_fields = frappe.get_all("Custom Field", filters={"fieldtype": "Link", "options": doctype_name}, fields=["dt as parent", "fieldname"])
