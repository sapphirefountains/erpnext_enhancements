...
- ~~Change orders are not first-class~~ — **closed in v1.459.0.** `Change Order` is a submittable DocType with a required `cause`, per-project numbering, a signed cost impact, and acceptance criteria that flow onto the inspection for the milestone they name. What remains open is *leakage*: nothing forces a verbal scope change to become a Change Order, so the count is only as complete as the discipline behind it.
