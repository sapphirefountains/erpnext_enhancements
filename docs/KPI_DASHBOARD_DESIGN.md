...
- **Definition:** Count of unscheduled counter purchases from Suppliers ticked *Store-Run Vendor*, in the last 30 days, **one per trip whichever record holds it** (v1.535.0, `metrics.combine_store_runs`). Two kinds of record:
...
- **Definition:** Stock Scan Log rows flagged `needs_review`, not reviewed and not undone: Add Without PO saves and, since v1.535.0, every store-run line. Called *Adds Without PO Awaiting Review* until v1.535.0; the key (`stock_scan_review_queue`) is unchanged, so a KPI Target set against it keeps grading.
