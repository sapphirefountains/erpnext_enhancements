...
- **Definition:** Count of unscheduled counter purchases from Suppliers ticked *Store-Run Vendor*, in the last 30 days, **one per trip whichever record holds it** (v1.536.0, `metrics.combine_store_runs`). Two kinds of record:
  - *Charges*: QuickBooks card purchases (vendor recovered from the raw payload's `EntityRef`, type Vendor, refunds excluded); submitted, non-return Purchase Invoices from those suppliers with no PO or receipt behind any line; and, so the figure does not go silent after the QuickBooks cutover, submitted Journal Entries **crediting** the store's payable on a line with no reference (one charge per entry, never a QuickBooks import; `metrics.journal_store_charges`). A debit to the store is a payment and never counts, and **Payment Entries are not a source at all**: a payment is money for a purchase already booked, so a bill and its payment booked unlinked would otherwise be two runs until someone reconciled them, and every nightly snapshot in between would keep the doubled figure.
  - *Recorded trips*: submitted, non-return Purchase Receipts from those suppliers with no PO line — the Stock Scan page's "Bought on a store run" writes one per line — grouped by run id (`custom_store_run`; a receipt without one is its own trip) or by receipt number at a store on a day. A trip's day is its earliest receipt; its amount is the receipt total the technician entered (tax included), else the sum of its receipts.
  - **Pairing:** each trip takes at most one charge at the same store (`stock_scan_rules.store_key`, so Lowes and Lowe's count as one) dated on the trip's day or up to 3 days after, never before — QuickBooks holds some purchases from both a receipt email and the bank feed, and a bank-feed entry carries the bank's posting date (Lowes $16.60 on the Capital One card: ACC-JV-2026-27340 from the receipt email, 2026-02-07; ACC-JV-2026-27137, "LOWES #02662* - 2486", from the feed, 2026-02-09), so the window lets a trip pair with a feed charge when that is its only charge. **Only on the amount**: first a charge equal to the receipt total, then one the lines plus tax could make (a mistyped total), nearest day first. A charge of any other amount is never taken. **Count** = trips + unpaired charges, each dated on or after the window start (rows are read from 7 days earlier, so a trip just before the window still claims its charge).
- **Expected jump:** when technicians start recording (from v1.536.0) the 30-day count will rise from 3 (measured 2026-09-24, $228.87) toward the real rate — 231 charges in the 12 months to 2026-09-24, about 19 a month. That is the measurement catching up with trips QuickBooks has not categorized yet (charges reach it about four weeks after the purchase), not more trips. With nothing recorded the figure is exactly the old one (231 runs, $19,551.55 over those 12 months, re-measured on production on 2026-09-24 with the QuickBooks arm's joins and filters).
- **After cutover:** the QuickBooks arm stops growing and ages out. The formula does not change. Accounting's side of it: a card charge at a store that has no recorded trip is booked as a standalone Purchase Invoice, or a Journal Entry crediting the store's payable, so it is seen here; a recorded trip is billed from its receipts (*Get Items From → Purchase Receipt*), and that invoice is the same trip, not a second one. A charge booked straight to an expense with no party is invisible here and would let the ≤4 target be met by not recording. Paying the invoice (a Payment Entry, or a Journal Entry debiting the store) never counts.
- **Known limits:** two identical QuickBooks charges on one store-day count twice (without a recorded trip there is no telling a duplicate from a second purchase), and so does a purchase QuickBooks holds from both the receipt email and the bank feed days apart (the Lowes $16.60 pair above), recorded or not, as in the baseline; a charge for a planned PO pickup still counts; two checkouts on one trip are two purchases; a charge more than 3 days after its trip counts as a second trip — bank-feed entries carry the bank's posting date, and same-amount, same-store pairs turn up 5, 7 and 11 days apart; a trip pairs **only** with a charge equal to its receipt total or within its lines plus 15% (the v1.536.0 review removed an "any amount in the window" pass: 163 of the 231 charges in the 12 months to 2026-09-24 have another at the same store within 3 days, so a recorded trip whose own charge never arrives — cash, a personal card, a Bill, an unflagged vendor — took the next trip's charge and two trips counted as one), so a trip whose card charge matches neither its receipt total nor its lines plus up to 15% (a mistyped total, a split payment) counts as two runs.
...
- **Data source:** Supplier.custom_store_run_vendor; QuickBooks Sync Mapping + QuickBooks Raw Payload + Journal Entry; Purchase Receipt (`custom_store_run`, `custom_receipt_total`, read behind `has_column`); Purchase Invoice; Journal Entry Account (credits only). Source label *Purchase Receipt + QuickBooks*, with **no freshness entry**, so a stale QuickBooks sync no longer greys out runs recorded today.
- **Implementation:** `_store_run_rows` (the SQL) and `_store_runs` in `kpi_dashboards/snapshots.py`, the count in `metrics.combine_store_runs` and, since v1.538.0, the pairing in `metrics.pair_store_runs`, which it calls (both pure, `tests/test_kpi_metrics.py`). The same pairing is listed trip by trip, with the card charge each took and which pass took it, by the **Store Run Charge Matching** report (KPI Dashboards), Accounting's list for the QuickBooks cutover, which pairs exactly as the KPI does when counted from the same From Date except where a charge's Reference Number links it to its trips by run id (the report only; the KPI's count is unchanged); see `kpi_dashboards/README.md`. Charges are read in a fixed order (posting date, voucher name) so an exact tie always goes to the same charge. Semi because a person decides which suppliers are counters. Not published until one is ticked — 0 by construction would read as the goal met.
- **Refresh:** Nightly. Recorded runs count the same day; unrecorded ones as QuickBooks categorization catches up.

### 2. Store-Run Spend (30d) — 🟡 Semi
- **Definition:** Dollars on the runs above: the card charge for a paired trip (tax included), the receipt total the technician entered for a trip whose charge has not arrived (else the sum of its receipts, before tax), and every unpaired charge.
- **Target:** Track only.
...
### 7. Stock Scan Saves Awaiting Review — 🟢 Auto
- **Definition:** Stock Scan Log rows flagged `needs_review`, not reviewed and not undone: Add Without PO saves and, since v1.536.0, every store-run line. Called *Adds Without PO Awaiting Review* until v1.536.0; the key (`stock_scan_review_queue`) is unchanged, so a KPI Target set against it keeps grading.
