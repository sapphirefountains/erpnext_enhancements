...
- **Implementation:** `_store_run_rows` (the SQL) and `_store_runs` in `kpi_dashboards/snapshots.py`, the pairing in `metrics.combine_store_runs` (pure, `tests/test_kpi_metrics.py`). Semi because a person decides which suppliers are counters. Not published until one is ticked — 0 by construction would read as the goal met.
