...
- **Definition:** Count of unscheduled counter purchases from Suppliers ticked *Store-Run Vendor*, in the last 30 days, **one per trip whichever record holds it** (v1.535.0, `metrics.combine_store_runs`). Two kinds of record:
  - *Charges*: QuickBooks card purchases (vendor recovered from the raw payload's `EntityRef`, type Vendor, refunds excluded); submitted, non-return Purchase Invoices from those suppliers with no PO or receipt behind any line; and, so the figure does not go silent after the QuickBooks cutover, submitted Journal Entries naming the store as a party on a line with no invoice reference (one per entry, never a QuickBooks import) and Payment Entries paying the store with no invoice reference.
  ...
  - **Pairing:** each trip takes at most one charge at the same store (`stock_scan_rules.store_key`, so Lowes and Lowe's count as one) dated on the trip's day or up to 3 days after, never before — QuickBooks holds some purchases from both a receipt entry and the bank feed, the feed up to two days later (Home Depot $43.31: 2026-07-07 and 2026-07-09), so exact-day matching counted such a trip twice. First a charge equal to the receipt total, then one the lines plus tax could make, then any in the window, nearest day first. **Count** = trips + unpaired charges, each dated on or after the window start (rows are read from 7 days earlier, so a trip just before the window still claims its charge).
...
- **After cutover:** the QuickBooks arm stops growing and ages out. The formula does not change. Accounting's side of it: every card charge at a store is booked with the store as the party — a Purchase Invoice (from the store-run receipts where there are any, standalone where there are not) or a Journal Entry / Payment Entry naming the store. A charge booked straight to an expense with no party is invisible here and would let the ≤4 target be met by not recording.
- **Known limits:** two identical QuickBooks charges on one store-day count twice (without a recorded trip there is no telling a duplicate from a second purchase); a charge for a planned PO pickup still counts; two checkouts on one trip are two purchases; a charge more than 3 days after its trip counts as a second trip.
