...
- ~~This site uses the single **`User.role_profile_name`** field (no multi `role_profiles`).~~ **Corrected 2026-07-28 (WI-066):** this is wrong. `tabUser Role Profile` exists and **multiple role profiles per user are already in live use** — Brian Morisseau holds `Sales` + `Sales Team`, Lian Silva `Technician` + `Production Team`, Logan Penrod `Design Team` + `Production Team`, Rich Hansen `Sales & Marketing` + `Sales Team`. Frappe **unions** the roles of every assigned profile. WI-066 depends on this: a user who carries a profile cannot hold a direct role durably (see next bullet), so the only way to give Clegg and Lisa `PO Creator` is a second, single-role profile.
- **Role changes must be made in the Desk UI, not the API.** MCP `update_document` can only edit `User.roles` when the user has **no** Role Profile assigned; while a profile is set, role edits are **silently discarded** on save and the profile field **cannot be cleared** via the API (both empty-string and `null` updates report success but do nothing). Reserve the API for reads.
  - **Root cause (found in WI-066):** `User.validate` calls `populate_role_profile_roles`, which — whenever at least one role profile is set — rebuilds `roles` from the union of those profiles on **every** save. Direct roles are dropped, not merged. The Desk enforces the same rule visually by disabling the Roles grid once a profile is assigned. So this is not an API quirk to work around: for a profiled user, **a role can only be granted through a profile**.
