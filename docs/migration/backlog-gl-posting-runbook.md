...
| **S-D** | Submit the ~13,068 **Journal Entries** (only after §3 B2 group-account lines == 0). **Before the loop, take out every draft QuickBooks Purchase JE that pairs with a store run recorded on the Stock Scan page** (v1.536.0): same store, dated on the trip's day or up to 3 days after, for the receipt total or the lines plus tax. For each, move the goods' debit to `2210 - Stock Received But Not Billed - SF` for the trip's stock lines before tax, then submit it; the tax and any non-stock line stay on the expense account. Submitted unchanged, it expenses goods the receipt already put into stock and leaves 2210 open. See CHANGELOG `[1.536.0]`, "For Accounting" note 2. | Accounts | GL grows to ~34,000; submitted JE count == expected; failure list empty; 2210 carries no store-run balance. |
