...
| **S-D** | Submit the ~13,068 **Journal Entries** (only after §3 B2 group-account lines == 0). **First clear the store runs with the *Store Run Charge Matching* report** (KPI Dashboards, v1.538.0): 1. Run it with **From Date 2026-01-01** and To Date today. 2. Work through **Needs action**, then **Waiting**, doing what each row's *What to Do* says. Finish a list before refreshing, and save each adjusted draft without submitting it. 3. Refresh and repeat until neither list changes, *Needs action* is empty and *2210 Not Accounted For* reads $0.00. 4. Run the loop. 5. Run the report again: a row now reading *Submitted with the goods on the expense* gets the correcting Journal Entry it names. Never amend a QuickBooks JE. Reference Number tokens, correcting entries and every rule: [MIGRATION_NOTES §8](../../erpnext_enhancements/quickbooks_online/MIGRATION_NOTES.md#8-store-runs-at-the-cutover-step-s-d). | Accounts | Before the loop: the report's lists no longer change, *Needs action* is empty, *2210 Not Accounted For* reads $0.00. After: GL grows to ~34,000; submitted JE count == expected; failure list empty; the report's *Needs action* is still empty and *2210 Not Accounted For* still $0.00. |
