...
| **S-D** | Submit the ~13,068 **Journal Entries** (only after §3 B2 group-account lines == 0). **Before the loop, run the *Store Run Charge Matching* report** (v1.538.0, KPI Dashboards) with **From Date 2026-01-01** (every 2026 draft: a Purchase Receipt with no PO at a flagged store counts as a trip whatever its date, and the Stock Scan page can post a run as yesterday, so the page's first day is not a safe start), To Date today and **Show = Needs action**. Each row is a recorded store run and the draft QuickBooks Purchase JE it pairs with, paired exactly as the Store Runs KPI pairs them (same store, dated on the trip's day or up to 3 days after, for the receipt total or the lines plus tax) unless a Reference Number links it (below), or a charge that carries 2210 but is neither paired nor linked (link it to its trip, unless that trip's row already shows another charge, or move it back to the expense), or a Journal Entry whose Reference Number names nothing usable or whose 2210 amount no charge accounts for (its row says why; put `not-store-run` in the Reference Number of an entry on 2210 that has nothing to do with store runs; a QuickBooks card charge must first carry nothing on 2210, or it stays listed), or a charge whose trips' stock lines add up to more than the charge itself (one of those trips is not its: take that trip's run id out). Adjust each listed draft as its *What to Do* column says: move the goods' debit to `2210 - Stock Received But Not Billed - SF` for the amount in *Stock Lines Before Tax (Move to 2210)*, which is what the trip's receipts credited to 2210 (split a line if need be), leave the tax and any non-stock line on the expense account, **then save it; the loop submits it** (do not submit drafts one by one). Re-run the report until *Needs action* lists nothing; an adjusted draft then reads *Goods debit on 2210 …: nothing to change; the S-D loop submits it*. **Then set Show = Waiting and check every trip dated on or before the last QuickBooks sync**: its charge did not pair (a bank-feed date more than 3 days late, an amount outside the tolerance, two runs of one purchase, a vendor not ticked *Store-Run Vendor*). Its row says one of two things, never both: **it has a card charge** — find the draft, move the trip's stock lines to 2210, **put the trip's run id in the draft's Reference Number** and save it (the report then shows the draft as the trip's charge, Match Basis *Linked by Reference Number*; when one draft pays for several trips, list all their run ids, separated by commas or spaces, keeping any already there, and the draft carries their stock lines together; if the draft is already another trip's charge in the list, list both run ids only if it pays for both, and otherwise this trip's alone, taking the other trip's out, so that trip's row asks for its own charge); or, **only if it has no card charge at all**, bill the trip from its receipts after the cutover. **A draft found under a QuickBooks vendor that is not a ticked Store-Run Vendor is invisible to the report until it is linked: link it by Reference Number anyway, and never bill that trip from its receipts** (the report reads every Journal Entry's Reference Number, whatever its vendor). **Then re-run with Show = Needs action, then Show = Waiting, and repeat until neither list changes, *Needs action* is empty and the summary's *2210 Not Accounted For* reads $0.00** (a link can move rows back into either list: a draft now carrying more than its linked trips' stock lines, or one the pairing had given another trip, whose row asks first whether the draft pays for that trip too, or for that trip and not the one it links, while that trip waits under *Waiting*, asked the same way; work through every row of a list before refreshing, since a question about a charge goes away once that charge is linked elsewhere; *2210 Not Accounted For* is every Journal Entry line on 2210 from 7 days before From Date to To Date that no charge accounts for). Only then run the loop. Submitted unchanged, a matching draft expenses goods the receipt already put into stock and leaves 2210 open. **Never amend a QuickBooks JE to fix one**: amending cancels the original, `tabQuickBooks Sync Mapping` stays on the cancelled entry, and the pairing follows the mapping, so the charge drops out of the report. A charge already submitted is fixed by posting and submitting **one correcting Journal Entry** (Dr 2210 / Cr the expense account the charge used) whose **Reference Number is the charge's name** followed by the run ids of the trips it pays for, as the row gives it, which links them (a submitted entry's own Reference Number cannot be changed) — and the report counts it once submitted (a draft correcting entry is listed until it is submitted or deleted; a Reference Number that names a correcting entry counts for that entry's charge). See CHANGELOG `[1.536.0]`, "For Accounting" note 2, and `[1.538.0]`. | Accounts | Before the loop: after repeating the Needs action and Waiting passes until neither list changes, the report's *Needs action* is empty, its *2210 Not Accounted For* reads $0.00, and every *Waiting* trip dated up to the last sync has its draft found, adjusted and linked, or is confirmed to have no card charge. After: GL grows to ~34,000; submitted JE count == expected; failure list empty; the report's *Needs action* is still empty (a draft submitted before it was adjusted reappears there as *Submitted with the goods on the expense*: post and submit the one correcting Journal Entry it names, with the Reference Number the row gives (the charge, then its trips' run ids), and the row moves to *Done*); its *On 2210 With No Store Run* and *2210 Not Accounted For* read $0.00; 2210 carries no store-run balance. |
