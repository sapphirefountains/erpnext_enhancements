...
| **S-D** | Submit the ~13,068 **Journal Entries** (only after §3 B2 group-account lines == 0). **Before the loop, run the *Store Run Charge Matching* report** (v1.538.0, KPI Dashboards) with **From Date 2026-09-24** (the first day a store run could be recorded on the Stock Scan page), To Date today and **Show = Needs action**. Each row is a recorded store run and the draft QuickBooks Purchase JE it pairs with, paired exactly as the Store Runs KPI pairs them (same store, dated on the trip's day or up to 3 days after, for the receipt total or the lines plus tax). Adjust each listed draft as its *What to Do* column says: move the goods' debit to `2210 - Stock Received But Not Billed - SF` for the amount in *Stock Lines Before Tax (Move to 2210)*, which is what the trip's receipts credited to 2210 (split a line if need be), and leave the tax and any non-stock line on the expense account. Re-run the report until *Needs action* lists nothing; an adjusted draft then reads *Goods debit on 2210: submit* and goes through the loop with the rest. Submitted unchanged, a matching draft expenses goods the receipt already put into stock and leaves 2210 open. See CHANGELOG `[1.536.0]`, "For Accounting" note 2, and `[1.538.0]`. | Accounts | Before the loop: the report's *Needs action* is empty. After: GL grows to ~34,000; submitted JE count == expected; failure list empty; the report's *Needs action* is still empty (a draft submitted before it was adjusted reappears there as *Submitted with the goods on the expense*); 2210 carries no store-run balance. |
