...
| [google-chat-teardown.md](google-chat-teardown.md) | The half of the Google Chat removal the code cannot do: the console work to retire the Chat app, the Workspace Events subscriptions, Pub/Sub, the delegation grant and the IAM binding — none of it under Terraform — plus which project and which scopes must be left alone so Drive and Calendar keep working. Companion to [ADR 0011](../decisions/adr/0011-retire-google-chat-and-coworker-chat.md) |
...
| [kiosk-native-app-spike.md](kiosk-native-app-spike.md) | Why a browser cannot track location with the screen off, what a Capacitor shell with a background-location plugin would add and cost, and the decision gate before [WI-076](../work-items/WI-076-kiosk-native-app.md) is built. Design only |
...
| [marketing-connectors-runbook.md](marketing-connectors-runbook.md) | The read-only Google Ads / Meta / LinkedIn spend connectors: registering each app, connecting with a read-only user, turning it on, and what each failure means |
