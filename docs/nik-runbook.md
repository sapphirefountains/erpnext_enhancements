...
**Four pull requests are open. None is merged; `main` auto-deploys to production, so merging is
yours.**
...
| [#829](https://github.com/sapphirefountains/erpnext_enhancements/pull/829) | erpnext_enhancements | 1.288.4 | The invocation log's cost columns were confidently wrong |
| [#340](https://github.com/sapphirefountains/triton/pull/340) | triton | 0.68.0 | The cached-token share, from provider to response |
| [#341](https://github.com/sapphirefountains/triton/pull/341) | triton | 0.68.1 | A drift guard that had skipped for its entire life |

**Merge order that matters:** `#341` is stacked on `#340` — merge `#340` first or the diff is
wrong. `#829` is harmless on its own but only *does* anything once `#340` is deployed, because it
reads a field Triton does not publish until then. `#828` and `#829` chose different version
numbers deliberately, so they do not collide.
