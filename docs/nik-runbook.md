...
| TASK-2026-01472 |  | Enable lead attribution, in the runbook's staged order | Follow the **staged checklist** in [attribution-runbook.md](attribution-runbook.md#the-staged-checklist) (v1.502.1). Note the corrected stage 1: the master switch goes on **together with the website ingress**, because alone it changes nothing. Record the baseline first; each stage has an Attribution Gaps check, an exit criterion and a rollback that leaves the ingress alone. |
