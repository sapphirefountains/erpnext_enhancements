...
* **Client address (`configs/nginx-realip.conf`)**: the load balancer connects from those same ranges and *appends* to `X-Forwarded-For`, so without help nginx — and therefore frappe's `request_ip` — sees a Google Front End as every visitor, and every IP-keyed rate limit becomes one global bucket (the state from 2026-07-18 to 2026-08-02). `startup_script.sh` installs this file as `/etc/nginx/conf.d/00-realip.conf` on every boot, testing it with `nginx -t` and rolling back on failure. It trusts both front-end ranges **and each load balancer's forwarding-rule address** — the latter is easy to miss and without it nginx records the load balancer itself. **If a forwarding-rule address changes, add the new one there and to `LB_FRONTEND_ADDRESSES` in `erpnext_enhancements/utils/client_ip.py`**; `tests/test_client_ip.py` fails the build if the two lists differ, and a daily check in the app writes an Error Log row if logins start arriving from the load balancer.
...
> **Day-to-day operations** → see [WORKFLOW.md](WORKFLOW.md) (quick start, file map, daily workflow, troubleshooting)  
> **Project-specific values** → see [GCP_SETUP.md](docs/GCP_SETUP.md) (actual IPs, domains, VM names, architecture)
