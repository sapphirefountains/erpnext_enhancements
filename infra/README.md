...
Deploys Cloud Build (v2) connections and 13 triggers securely, utilizing Google Secret Manager to store GitHub Personal Access Tokens (PATs) for repository mirroring and validation. Triggers include 9 manual infra triggers (`infra-{shared,test,prod}-{apply,destroy,refresh}`) and 4 app triggers (`app-deploy-{test,prod}`, `app-upstream-{test,prod}`).
...
**How it works:**
- **13 triggers** created in Cloud Build:
  - 9 manual infra triggers: `infra-{shared,test,prod}-{apply,destroy,refresh}`
  - 2 auto app deploy triggers: `app-deploy-test`, `app-deploy-prod` (prod is disabled by default)
  - 2 manual upstream triggers: `app-upstream-test`, `app-upstream-prod`
- Each app trigger SSHes into the target VM via IAP as the `deploy` user, runs bench commands, and restarts `frappe-bench.service`.
...
## Three-State Deployment Model

The infrastructure is split across **three isolated Terraform states** to separate concerns and allow independent management of each environment.

| State | Backend Config | Variable File | Manages |
|-------|---------------|---------------|---------|
| **shared** | `backend-shared.hcl` | `shared.tfvars` | Project, VPC, NAT, IAM, Cloud Build connection, IPs, SSL, Load Balancers, firewalls, health checks, state bucket |
| **test** | `backend-test.hcl` | `test.tfvars` | Spot VM, spot VM disks, snapshot restores |
| **prod** | `backend-prod.hcl` | `prod.tfvars` | Standard VM, prod VM boot/data disks |

Each state uses the `deployment_mode` variable (`shared`/`test`/`prod`) to conditionally skip resources owned by other states. The project module always runs (reads existing project via data source when `create_project = false`), so `module.project.project_id` is available in all states.
...
```bash
# Shared infrastructure (project, VPC, IAM, CB, IPs, LB, SSL)
terraform apply -var-file=shared.tfvars

# Test environment (spot VM)
terraform apply -var-file=test.tfvars

# Production environment (standard VM)
terraform apply -var-file=prod.tfvars
```

### Migrating from Single-State

If the current `terraform.tfvars` state owns all resources (including VMs), migrate test/prod resources to their dedicated backends:

**Option A — Destroy and recreate (simpler, brief downtime):**
1. Apply `shared.tfvars` with `provision_spot_vm = false` and `provision_compute_vm = false` to remove VMs from shared state.
2. Apply `test.tfvars` and `prod.tfvars` to recreate VMs in their own states.

**Option B — State mv (no downtime, but more commands):**
```bash
# Move spot VM resources to test state
terraform state mv -state-out=test.tfstate \
  module.spot_vm["test-erpnext-spot-vm"] \
  module.spot_vm["test-erpnext-spot-vm"]

# Move prod VM resources to prod state
terraform state mv -state-out=prod.tfstate \
  module.compute_vm["production-erpnext-standard-vm"] \
  module.compute_vm["production-erpnext-standard-vm"]
```

### CI/CD Note

The Cloud Build pipeline (`cloudbuild.yaml`) uses `$_DEPLOYMENT_MODE` substitution to select the correct backend prefix (`terraform/$_DEPLOYMENT_MODE`) and var-file (`$_DEPLOYMENT_MODE.tfvars`). For test/prod runs, the var-file is read from the repo. For shared runs, `SECRET_TFVARS` from Secret Manager is written to `shared.tfvars` first (because it contains the GitHub token). Keep `SECRET_TFVARS` in sync with your local `shared.tfvars`.
...
# Initialize per-environment backends (one-time)
terraform init -backend-config=backend-shared.hcl -reconfigure
terraform init -backend-config=backend-test.hcl -reconfigure
terraform init -backend-config=backend-prod.hcl -reconfigure
...
Each service can be toggled independently via boolean variables in the appropriate `.tfvars` file. Set the toggle in the file matching the service's state:

| Service | Toggle | Set in |
|---------|--------|--------|
| Shared resources (project, VPC, NAT, IAM, IPs, SSL, LB, Cloud Build) | `provision_*` | `shared.tfvars` |
| Spot VM, test disks, snapshots | `provision_spot_vm` | `test.tfvars` |
| Standard VM, prod disks | `provision_compute_vm` | `prod.tfvars` |

Specific options are tailored via YAML files in the `configs/` directory.

### Enabling Services
Edit the relevant `.tfvars` file and set the following toggles to `true` or `false`:
...
In `shared.tfvars`, set the provisioning toggle:
```hcl
...
In `shared.tfvars`, enable Cloud Run and specify the container image path pointing to your newly created registry:
```hcl
...
In `shared.tfvars`, enable the load balancer and spot VM LB backend:

```hcl
provision_load_balancer          = true
provision_spot_vm_lb_backend     = true
provision_standard_vm_lb_backend = false   # disable prod backend if not needed
```
...
#### 2. Enable Services and Configure Disks in Test

In `test.tfvars`, set the provisioning toggles and disk settings:
...
#### 3. Run Terraform Apply for Test
...
|---|---|---|---|---|
| [deployment_mode](variables.tf#L17) | Controls which resources are managed by this state: 'shared', 'test', or 'prod'. | <code>string</code> |  | <code>&#34;shared&#34;</code> |
| [project_id](variables.tf#L133) | The ID of the project to create or reuse. | <code>string</code> | ✓ |  |
...
| [deployment_mode](outputs.tf#L87) | The active deployment mode. |  |
| [network_id](outputs.tf#L72) | The VPC network self-link. |  |
| [network_name](outputs.tf#L67) | The VPC network name. |  |
| [project_number](outputs.tf#L61) | The numeric GCP Project number. |  |
| [spot_vms](outputs.tf#L100) | The outputs of provisioned Spot VMs. |  |
| [sql_instances](outputs.tf#L108) | The outputs of the provisioned Cloud SQL database instances. | ✓ |
| [ssl_certificates](outputs.tf#L116) | The outputs of the SSL Certificates Manager configuration. |  |
| [state_bucket_name](outputs.tf#L82) | The GCS bucket name for Terraform state. |  |
| [subnetwork_self_link](outputs.tf#L77) | The subnetwork self-link. |  |
