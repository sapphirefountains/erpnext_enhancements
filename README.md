...
- **Time tracking** — a standalone, installable **Time Kiosk PWA** (light / dark / system theme) with battery-aware GPS tracking, a deliberate high-accuracy fix at every clock event, per-session tracking health, offline queueing, and automatic consolidation into ERPNext Timesheets carrying the employee's burdened labour cost.
...
| **HR Enhancements** (`hr_enhancements/`) | The `Position` ladder (job families x tier) that sign-off authority is read from, the company tree, and the one desk area an employee opens to find their own record. Labelled **HR** in the UI; not named `HR` because hrms ships a module by that name | [README](erpnext_enhancements/hr_enhancements/README.md) |
| **Quality** (`quality/`) | Project inspections, non-conformance and corrective action. Labelled **Quality Control** in the UI; not named `Quality` because erpnext's Quality Management module already owns a workspace and a Desk tile by that name | [README](erpnext_enhancements/quality/README.md) |
...
| **Knowledge Base** (`knowledge_base/`) | The company knowledge base ([WI-080](work-items/WI-080-company-knowledge-base.md), [ADR 0017](decisions/adr/0017-company-knowledge-lives-in-a-native-module.md)): `Knowledge Article`, the approved text every staff user reads, and `Knowledge Article Version`, the drafts and history only KB Authors and KB Approvers can open, with approval by a second person enforced in code. **PRs 1 and 2 only**: the doctypes, the roles and the locked permissions, then the approval and content rules and private Files. Nothing can publish yet | [README](erpnext_enhancements/knowledge_base/README.md) |
| **Inventory Enhancements** (`inventory_enhancements/`) | Barcode count sessions, storage locations, scanner audit; the **Stock Scan** page (`/stock-scan`: scan a location's QR label, then take parts, receive against the open PO, or put stock away — each save a submitted voucher, with undo) and its printable warehouse QR labels (`/warehouse-labels`) | [README](erpnext_enhancements/inventory_enhancements/README.md) |
| **Workforce** (`workforce/`) | Time Kiosk pages, Job Interval clock-in sessions, the Location Timeline (trail playback + live map), effective-dated pay rates and labour costing, time-correction requests, the forgotten-clock-out sweeper, the supervisor digest, the payroll workbook | [README](erpnext_enhancements/workforce/README.md) |
...
| **Offsite Backup** (`offsite_backup/`) | Verified full backups (database + files) to a Google Drive Shared Drive every night, with retention and a staleness watchdog | [README](erpnext_enhancements/offsite_backup/README.md) |
...
> **Retired in v1.426.0 — the `chat/` module.** It was employee chat mirrored bidirectionally with Google Chat: rooms, threaded messages, an outbox relay, the inbound Workspace Events pipeline, an SPA at `/chat`, and a coworker surface in the floating bubble — 23 DocTypes and ~58k lines of Python. It shipped dormant (`Chat Settings.enabled = 0`) and was never enabled on any site, so nothing a coworker wrote was lost. The reasoning is worth keeping, because it decided the shape of what is left: the mirror was the *only* thing that gave the ERPNext-side chat product an audience — staff use the native Google Chat app on their phones — so retiring the buggy transport retired the product with it rather than leaving a smaller chat product behind. What ERPNext keeps is **chatting with Triton**, which was always a separate surface: the floating Desk widget (`public/js/global_enhancements/triton_widget.js`), its server side (`triton_chat.py`, `triton_attachments.py`), the `Triton Chat Attachment` doctype in AI Governance, and three browser modules re-homed from `public/js/chat/` to `public/js/triton/`. See [ADR 0011](decisions/adr/0011-retire-google-chat-and-coworker-chat.md) — which supersedes [ADR 0009](decisions/adr/0009-erpnext-google-chat-triton.md) and its addenda, now history rather than instruction — and [`docs/google-chat-teardown.md`](docs/google-chat-teardown.md) for the GCP-side teardown that must be done by hand.
...
| `override_whitelisted_methods` | Replace a core endpoint | `opportunity.make_project` → `opportunity_enhancements.make_project`; `frappe.utils.print_format.download_pdf` → `po_pdf_filename.download_pdf` (the Project Number in the downloaded PO filename — frappe builds that name from the docname and never consults `title_field`, so this is the only lever on it); `erpnext…timesheet.get_activity_cost` → `api.activity_cost.get_activity_cost` (v1.542.0: `costing_rate` is a burdened pay rate, zeroed over HTTP for anyone outside ADR 0013's pay audience; `billing_rate` and the server-side `TimesheetDetail.update_cost` caller are untouched); eleven frappe core routes → `fieldlevel_read.*` (v1.542.0: `getdoc`, `get_docinfo`, `savedocs` and the Desk's `cancel` / `discard` send the stored Version diffs verbatim, and `frappe.client`'s writes plus `apply_workflow` send back the unstripped document, so each wrapper calls frappe's function unchanged and drops the fields the caller cannot read at their permlevel) |
| `after_request` | Post-process every response | `fieldlevel_read.scrub_rest_write_response` (v1.542.0): `POST`/`PUT /api/resource` and `/api/v2/document` return the saved document without field-level read permissions, and those are routes no override reaches, so the JSON body is scrubbed here. Every other request leaves on a method + path check |
| `before_request` | Run before every request | `fieldlevel_read.seal_wrapped_originals` (v1.542.0): an override matches a method *name* but frappe's whitelist matches the function *object*, so modules that import one of the eleven wrapped functions (`frappe.email.inbox.set_value`, Workflow Action's `apply_workflow`, several shipped test modules) exposed the unscrubbed original under their own path. This takes the originals off frappe's whitelist once their canonical names are overridden, so every alias fails the whitelist check while the canonical names reach the wrappers. Never raises |
...
- **Time Kiosk PWA** — `/kiosk`, an installable offline-capable PWA. The front-end (`public/js/kiosk/`) samples GPS with `watchPosition` + distance filter + heartbeat while clocked in and takes a deliberate high-accuracy anchor fix at every clock event; the service worker (`www/kiosk-sw.js`) queues points in IndexedDB and batch-uploads with Background Sync. A browser cannot track with the screen off on any phone — [`docs/kiosk-native-app-spike.md`](docs/kiosk-native-app-spike.md) is the gated design for the day that matters. → [www README](erpnext_enhancements/www/README.md) and [public README](erpnext_enhancements/public/README.md#kiosk-pwa-front-end)
...
- **Operations:** [Sapphire Maintenance](erpnext_enhancements/sapphire_maintenance/README.md) · [Fleet Maintenance](erpnext_enhancements/fleet_maintenance/README.md) · [Travel Management](erpnext_enhancements/travel_management/README.md) · [Asset Management](erpnext_enhancements/asset_management/README.md) · [Package Dispatch](erpnext_enhancements/package_dispatch/README.md) · [Inventory Enhancements](erpnext_enhancements/inventory_enhancements/README.md) · [Workforce](erpnext_enhancements/workforce/README.md) · [HR Enhancements](erpnext_enhancements/hr_enhancements/README.md) · [Training](erpnext_enhancements/training/README.md) · [Knowledge Base](erpnext_enhancements/knowledge_base/README.md) · [Quality](erpnext_enhancements/quality/README.md)
